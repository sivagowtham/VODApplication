...
public class MovieManagementServiceImpl implements MovieManagementService {
	...
	@Override
	public boolean addMovie(MovieDTO movieDTO) throws ServiceException {
		logger.debug("addMovie " + movieDTO.getTitle());
		boolean flag = false;
		try {
			List<String> movieTitles = movieDAO.retriveAllMovieTitles();
			if(!movieTitles.contains(movieDTO.getTitle()))
			{
			...
			{
				logger.info("Movie with same title already available in the Database" + movieDTO.getMovieId());
			}
			return flag;
