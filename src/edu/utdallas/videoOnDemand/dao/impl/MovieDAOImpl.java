...
public class MovieDAOImpl extends BaseDAO implements MovieDAO {
	...
	private final String insertMovieQuery = "INSERT INTO TBL_MOVIES (TITLE,DESCRIPTION,DIRECTOR,ACTORS,CATEGORY_ID,ADDED_ON,POSTER_URL,VIDEO_URL,IMDB_ID,IS_VISIBLE) VALUES (?,?,?,?,?,?,?,?,?,?)";
	...
	public void insertMovie(Movie movie) throws DAOException {
		...
		Object values[] = { movie.getTitle(), movie.getDescription(),
				movie.getDirector(), movie.getActors(), movie.getCategoryID(),
				date, movie.getPosterURL(), movie.getVideoURL(),
				movie.getImdbId(),movie.getIsVisible() };
		jdbcTemplate.update(insertMovieQuery, values);
	}

	private final String updateMovieQuery = "update tbl_movies set TITLE=?, DESCRIPTION=?, DIRECTOR=?, ACTORS=?, CATEGORY_ID=?, RATING=?, VIDEO_URL=?, "
	...
	public void updateMovie(Movie movie) throws DAOException {
		...
		try {
			...
			try {
				...
				ps.setLong(5, movie.getCategoryID()); // CATEGORY
	...
	public void updateMovieRating(Movie movie) throws DAOException {
		...
		try {
			...
			try {
				...
				ps.setLong(3, movie.getMovieId()); // MOVIE_ID
	...
	private final String retrieveAllVisibleMoviesQuery = "SELECT * FROM TBL_MOVIES WHERE IS_VISIBLE = 'Y' ORDER BY ADDED_ON DESC";

	@Override
	public List<Movie> retrieveAllVisibleMovies() throws DAOException {
		logger.debug("Starting retrieveAllVisibleMovies");

		JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
		List<Movie> movies = jdbcTemplate.query(retrieveAllVisibleMoviesQuery,
				new MovieMapper());
		return movies;
	}

	private final String retrieveMoviesByCategoryQuery = "SELECT * FROM TBL_MOVIES WHERE CATEGORY_ID=? ORDER BY ADDED_ON DESC";

	@Override
	public List<Movie> retrieveMoviesByCategory(Long categoryID) throws DAOException {
		logger.debug("Starting retrieveMoviesByCategory");
		if (categoryID == null) {
			throw new DAOException(
					"Required keys category id");
		}
		JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
		try {
			Object args[] = { categoryID };
			List<Movie> movies = jdbcTemplate.query(retrieveMoviesByCategoryQuery, args,
					new MovieMapper());
			return movies;
		} catch (EmptyResultDataAccessException ex) {
			return null;
	...
	private final String checkMoviePresentQuery = "Select count(*) from TBL_MOVIES where movie_id = ?";

	@SuppressWarnings("deprecation")
	public boolean checkMoviePresent(Long movieId)
	{
		boolean flag = false;
		JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
		Object[] params = { movieId };
		int[] types = { Types.INTEGER };
		Long moviePresent = jdbcTemplate.queryForLong(checkMoviePresentQuery, params, types);
		if(moviePresent>0)
		{
			flag = true;
		}
	...
	{
		boolean flag = false; 
		boolean moviePresent = checkMoviePresent(movieId);
		if(moviePresent==true)
		{
			flag = true;
			JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
			Object[] params = { movieId };
	...
	@Override
	public boolean showMovie(Long movieId)
	{
		boolean flag = false;
		boolean moviePresent = checkMoviePresent(movieId);
		if(moviePresent==true)
	...
	@Override
	public boolean hideMovie(Long movieId)
	{boolean flag = false;
	boolean moviePresent = checkMoviePresent(movieId);
	if(moviePresent==true)
	{
	...
	private final String retrieveRentedMovies = "SELECT m.MOVIE_ID,m.TITLE,m.DESCRIPTION,m.DIRECTOR,m.ACTORS,m.CATEGORY_ID,m.RATING,m.VIDEO_URL,m.IMDB_ID,m.ADDED_ON,m.RENT_AMOUNT,m.PURCHASE_AMOUNT,m.POSTER_URL,m.IS_VISIBLE,m.NUM_TIMES_RATED FROM TBL_MOVIES m JOIN TBL_RENTED_MOVIES r ON r.MOVIE_ID = m.MOVIE_ID WHERE r.USER_ID = ? ORDER BY r.RENTED_ID DESC";

	@Override
	public List<Movie> retrieveRentedMovies(Long userId) throws DAOException {
		logger.debug("Starting retrieveRentedMovies");
		if (userId == null) {
			throw new DAOException(
		...
		JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
		Object values[] = { userId };
		List<Movie> movies = jdbcTemplate.query(retrieveRentedMovies, values,
				new MovieMapper());
		return movies;
	}

	private final String retrievePurchaseMovies = "SELECT m.MOVIE_ID,m.TITLE,m.DESCRIPTION,m.DIRECTOR,m.ACTORS,m.CATEGORY_ID,m.RATING,m.VIDEO_URL,m.IMDB_ID,m.ADDED_ON,m.RENT_AMOUNT,m.PURCHASE_AMOUNT,m.POSTER_URL,m.IS_VISIBLE,m.NUM_TIMES_RATED FROM TBL_MOVIES m JOIN TBL_MOVIE_PURCHASED p ON p.MOVIE_ID = m.MOVIE_ID WHERE p.USER_ID = ? ORDER BY p.PURCHASED_ID DESC";

	@Override
	public List<Movie> retrievePurchaseMovies(Long userId) throws DAOException {
		logger.debug("Starting retrievePurchaseMovies");
		if (userId == null) {
			throw new DAOException(
					"UserId cannot be null");
		}
		...
		JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
		Object values[] = { userId };
		List<Movie> movies = jdbcTemplate.query(retrievePurchaseMovies, values,
				new MovieMapper());
		return movies;
	}
